...
const createPaymentSchema = z.object({
  ...
  method: z
    .enum(['CASH', 'BANK_TRANSFER', 'CREDIT_CARD', 'DEBIT_CARD', 'CHECK', 'OTHER', 'KHIPU'])
    .optional(),
  description: z.string().min(1, 'Descripción requerida').max(500, 'Descripción muy larga'),
  status: z
    .enum(['PENDING', 'COMPLETED', 'FAILED', 'REFUNDED', 'PARTIAL', 'OVERDUE'])
    .default('PENDING'),
});
...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.error('Error en consulta optimizada de pagos', {
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.error('Error creando pago', {
      error: error instanceof Error ? error.message : String(error),
    });
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (validatedData.amount !== undefined) {
      prismaUpdateData.amount = validatedData.amount;
    }
    if (validatedData.dueDate !== undefined) {
      prismaUpdateData.dueDate = new Date(validatedData.dueDate);
    }
    if (validatedData.method !== undefined) {
      prismaUpdateData.method = validatedData.method;
    }
    if (validatedData.description !== undefined) {
      prismaUpdateData.description = validatedData.description;
    }
    if (validatedData.status !== undefined) {
      prismaUpdateData.status = validatedData.status;
    }
    if (validatedData.paidDate !== undefined) {
      prismaUpdateData.paidDate = new Date(validatedData.paidDate);
    }
    ...
    logger.error('Error actualizando pago', {
      error: error instanceof Error ? error.message : String(error),
    });
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    logger.error('Error eliminando pago', {
      error: error instanceof Error ? error.message : String(error),
    });
