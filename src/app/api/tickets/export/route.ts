...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.info('GET /api/tickets/export - Exportando tickets', {
      userId: user.id,
      userRole: user.role,
      format,
      statusFilter,
    ...
    // Filtrar por usuario (solo ver tickets propios a menos que sea admin/support)
    ...
    if (format === 'csv') {
      ...
      const response = new NextResponse(JSON.stringify(jsonData, null, 2), {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="tickets_${new Date().toISOString().split('T')[0]}.json"`,
        },
      });
      return response;
    } else {
