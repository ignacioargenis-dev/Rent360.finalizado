...
export async function GET(
  ...
  try {
    ...
    logger.error('Error al obtener comentarios:', { error: error instanceof Error ? error.message : String(error) });
...
export async function POST(
  ...
  try {
    ...
    logger.error('Error al crear comentario:', { error: error instanceof Error ? error.message : String(error) });
    if (error instanceof Error && error.message === 'No autorizado') {
      return NextResponse.json(
        { error: 'No autorizado' },
        { status: 401 },
