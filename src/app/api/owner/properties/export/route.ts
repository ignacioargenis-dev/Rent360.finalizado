  ...
  try {
    ...
    if (format === 'csv') {
      ...
      const csvRows = properties.map(property => [
        ...
        property.area || 0,
        `"${property.description?.replace(/"/g, '""') || ''}"`, // Escapar comillas en CSV
      ...
      const response = new NextResponse(csvContent, {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="propiedades_propietario_${new Date().toISOString().split('T')[0]}.csv"`,
        },
      });
      ...
      const jsonData = properties.map(property => ({
        ...
        activeContracts: property._count.contracts,
      ...
      const response = new NextResponse(JSON.stringify(jsonData, null, 2), {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="propiedades_propietario_${new Date().toISOString().split('T')[0]}.json"`,
        },
      });
    ...
    logger.error('Error exporting owner properties:', error);
