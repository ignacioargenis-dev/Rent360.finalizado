...
const DEFAULT_INTEGRATIONS = [
  ...
  {
    id: 'yoid',
    name: 'Yoid - Verificación Biométrica',
    description: 'Verificación de identidad biométrica, face matching y liveness detection',
    category: 'identity',
    isEnabled: false,
    isConfigured: false,
    isTested: false,
    config: {
      apiUrl: 'https://api.yoid.cl',
      apiKey: '',
      secretKey: '',
      environment: 'test',
      webhookUrl: 'https://rent360.cl/api/kyc/yoid/webhook',
    },
  },
  {
    id: 'verifik',
    name: 'Verifik - Validación de Identidad',
    description: 'Validación de RUT con Registro Civil y verificación de antecedentes',
    category: 'identity',
    isEnabled: false,
    isConfigured: false,
    isTested: false,
    config: {
      apiUrl: 'https://api.verifik.cl',
      apiKey: '',
      clientId: '',
      clientSecret: '',
      environment: 'test',
  ...
  {
    id: 'registro-civil',
    name: 'Registro Civil - Validación RUT',
    description: 'Validación oficial de RUT con base de datos del Registro Civil de Chile',
    category: 'identity',
    isEnabled: false,
    isConfigured: false,
    isTested: false,
    config: {
      apiUrl: 'https://api.registrocivil.cl',
      apiKey: '',
      institutionCode: '',
  ...
  },
  {
    ...
    description: 'Reconocimiento facial, análisis de documentos y liveness detection con AWS',
    category: 'identity',
    isEnabled: false,
    isConfigured: false,
    isTested: false,
    config: {
      ...
      secretAccessKey: '',
      region: 'us-east-1',
      collectionId: 'rent360-faces',
    },
  },
  {
    id: 'dicom-equifax',
    name: 'DICOM / Equifax',
    description: 'Consulta de historial crediticio y verificación financiera',
    category: 'identity',
    isEnabled: false,
    isConfigured: false,
    isTested: false,
    config: {
      apiUrl: 'https://api.equifax.cl',
      username: '',
      password: '',
      companyCode: '',
      serviceCode: 'INFOCOMER',
    },
  },
  {
    id: 'twilio',
...
async function updateEnvironmentVariables(integrationId: string, config: any) {
  ...
  const envMappings: Record<string, Record<string, string>> = {
    ...
    yoid: {
      YOID_API_URL: config.apiUrl,
      YOID_API_KEY: config.apiKey,
      YOID_SECRET_KEY: config.secretKey,
      YOID_ENVIRONMENT: config.environment,
      YOID_WEBHOOK_URL: config.webhookUrl,
    },
    verifik: {
      VERIFIK_API_URL: config.apiUrl,
      VERIFIK_API_KEY: config.apiKey,
      VERIFIK_CLIENT_ID: config.clientId,
      VERIFIK_CLIENT_SECRET: config.clientSecret,
      VERIFIK_ENVIRONMENT: config.environment,
    },
    'registro-civil': {
      REGISTRO_CIVIL_API_URL: config.apiUrl,
      REGISTRO_CIVIL_API_KEY: config.apiKey,
      REGISTRO_CIVIL_INSTITUTION_CODE: config.institutionCode,
      REGISTRO_CIVIL_CERTIFICATE_ENABLED: config.certificateEnabled,
    },
    'aws-rekognition': {
      AWS_REKOGNITION_ACCESS_KEY_ID: config.accessKeyId,
      AWS_REKOGNITION_SECRET_ACCESS_KEY: config.secretAccessKey,
      AWS_REKOGNITION_REGION: config.region,
      AWS_REKOGNITION_COLLECTION_ID: config.collectionId,
    },
    'dicom-equifax': {
      DICOM_API_URL: config.apiUrl,
      DICOM_USERNAME: config.username,
      DICOM_PASSWORD: config.password,
      DICOM_COMPANY_CODE: config.companyCode,
      DICOM_SERVICE_CODE: config.serviceCode,
    },
    twilio: {
