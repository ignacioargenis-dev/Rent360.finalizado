...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.error('Error en GET /api/admin/system-metrics:', { error: error instanceof Error ? error.message : String(error) });
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.error('Error en POST /api/admin/system-metrics:', { error: error instanceof Error ? error.message : String(error) });
...
async function getSystemMetrics() {
  ...
  try {
    ...
    const recommendations = generateRecommendations(dbStats, systemStats, cacheStats, rateLimitStats);
    ...
    logger.error('Error obteniendo métricas del sistema:', { error: error instanceof Error ? error.message : String(error) });
    throw error;
  }
}
...
async function getPerformanceStats() {
  try {
    ...
    return {
      ...
      avgResponseTime: slowQueries.length > 0 
...
async function getActiveAlerts() {
  ...
  try {
    ...
    logger.error('Error obteniendo alertas:', { error: error instanceof Error ? error.message : String(error) });
  }
...
function calculateHealthScore(dbStats: any, systemStats: any, cacheStats: any, rateLimitStats: any) {
  let score = 100;
  
  // Penalizar por problemas de base de datos
  if (dbStats.status !== 'healthy') score -= 30;
  if (dbStats.slowQueries > 10) score -= 10;
...
function generateRecommendations(dbStats: any, systemStats: any, cacheStats: any, rateLimitStats: any) {
