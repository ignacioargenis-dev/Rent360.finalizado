...
export async function GET(request: NextRequest) {
  try {
    ...
    if (format === 'csv') {
      ...
      const response = new NextResponse(csvContent, {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="contratos_inquilino_${new Date().toISOString().split('T')[0]}.csv"`,
        },
      });
      ...
      const response = new NextResponse(JSON.stringify(jsonData, null, 2), {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="contratos_inquilino_${new Date().toISOString().split('T')[0]}.json"`,
        },
      });
    ...
    logger.error('Error exporting tenant contracts:', error);
