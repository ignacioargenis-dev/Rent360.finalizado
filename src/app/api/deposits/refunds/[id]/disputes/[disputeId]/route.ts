...
export async function PUT(
  ...
  try {
    ...
    if (dispute.refundId !== refundId) {
      return NextResponse.json({ error: 'Disputa no pertenece a esta solicitud' }, { status: 400 });
    }

    // Verificar que la disputa está activa
    if (!['OPEN', 'UNDER_MEDIATION'].includes(dispute.status)) {
      return NextResponse.json({ error: 'La disputa ya ha sido resuelta' }, { status: 400 });
    }
