...
const createRefundSchema = z.object({
  ...
  tenantClaimed: z
    .number()
    .min(0, 'Monto reclamado por inquilino debe ser mayor o igual a 0')
    .default(0),
  ownerClaimed: z
    .number()
    .min(0, 'Monto reclamado por propietario debe ser mayor o igual a 0')
    .default(0),
...
const updateRefundSchema = z.object({
  ...
  status: z
    .enum(['PENDING', 'UNDER_REVIEW', 'DISPUTED', 'APPROVED', 'PROCESSED', 'REJECTED'])
    .optional(),
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (user.role !== 'ADMIN' && user.id !== contract.tenantId && user.id !== contract.ownerId) {
    ...
    const refund = await db.depositRefund.create({
      data: {
        ...
        refundNumber,
        originalDeposit: contract.depositAmount,
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (user.role === 'ADMIN') {
      ...
      if (validatedParams.contractId) {
        where.contractId = validatedParams.contractId;
      }
      if (validatedParams.tenantId) {
        where.tenantId = validatedParams.tenantId;
      }
      if (validatedParams.ownerId) {
        where.ownerId = validatedParams.ownerId;
      }
